...
boundaries, app architecture, workflows, gotchas). This file mirrors it for
agents that read `AGENTS.md`; when the two disagree, `CLAUDE.md` wins.
...
- **Model router** (`core/model-router.ts`): Per-turn model switching. Modes: `vision` (auto-switch on images/video/docs), `plan-execute` (heavy planner + light executor), `hybrid` (vision priority, then plan-execute). Vision fallback chain: GLM-4.6V → MiMo Omni → Moonshot → OpenAI (Codex excluded for cost).
...
Project-agnostic probes that let the agent _see_ what's happening in the running project (UI screenshots, runtime logs, HTTP responses, captured emails) and persist signals it would otherwise have to guess about.
...
tests. Release automation (`release.yml`) triggers on `v*` tags.
