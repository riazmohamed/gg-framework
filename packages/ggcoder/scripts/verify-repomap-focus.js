 ...
 *
 * Reproduces the "gg-voice is dirty but we're actively editing ggcoder" case
 * without restarting the CLI. Run:
 *   pnpm --filter @kenkaiiii/ggcoder verify:repomap
 */
...
const READ_FILES = [
  ...
  "packages/ggcoder/scripts/verify-edit-agent-view.ts",
  "packages/ggcoder/scripts/verify-edit-pomodoro.ts",
  "packages/ggcoder/src/core/agent-session.ts",
  "packages/ggcoder/src/core/repomap.test.ts",
  "packages/ggcoder/src/core/repomap.ts",
  "packages/ggcoder/tsconfig.json",
];
...
async function main() {
  const rendered = await buildRepoMap({
    ...
    focusTerms: [
      "Why is it focused on gg-voice? We're not even actively working on that. We're adjusting ggcoder right now.",
    ],
  ...
  record(
    ...
    paths.every((filePath) => !filePath.startsWith("packages/gg-voice/")),
  );
  record(
  ...
  record(
    ...
    paths[0] !== "packages/ggcoder/src/ui/App.tsx",
    paths[0] ?? "",
  );
