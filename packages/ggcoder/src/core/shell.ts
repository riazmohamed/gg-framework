 ...
 * Git Bash or msys2 install). `GG_GIT_BASH_PATH` is accepted as a fallback for
 * Windows users configured before the variable was renamed. Otherwise:
 * non-Windows → `bash`; Windows → Git Bash if found, else `cmd.exe`.
 */
export function resolveShell(command: string, opts: ResolveShellOpts = {}): ShellResolution {
  ...
  const override = (env.GG_BASH ?? env.GG_GIT_BASH_PATH)?.trim();
  if (override) {
