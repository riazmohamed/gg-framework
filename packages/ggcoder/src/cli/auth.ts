...
import { getXiaomiBaseUrl, type XiaomiRegion } from "../core/xiaomi-regions.js";
...
export async function runLogin(): Promise<void> {
  ...
  // Phase 1.5: Xiaomi Token Plan keys are region-scoped — a key issued for one
  // region returns 401 on another. Capture the region now, while stdin is still
  // free: this is a raw-mode Ink selector and cannot coexist with an active
  // readline interface. (API Credits uses a single global endpoint, so the
  // region only applies to the Token Plan path below.)
  let xiaomiRegion: XiaomiRegion | null = null;
  if (provider === "xiaomi") {
    xiaomiRegion = await renderXiaomiRegionSelector();
    if (!xiaomiRegion) {
      console.log(chalk.hex("#6b7280")("Login cancelled."));
      return;
    }
