...
/**
 * Fork-only: open improvement signals from past use of this project's
 * perception probes (`.gg/eyes/`). Gated on `.gg/eyes/manifest.json`, so the
 * section is absent for every project that has not opted into Eyes.
 */
function renderEyesSection(cwd: string): string | null {
...
export async function buildSystemPrompt(
  ...
  const eyesSection = renderEyesSection(cwd);
  if (eyesSection) sections.push(eyesSection);
