...
export function useAgentLoop(
  ...
  /** Model the vision/plan-execute router last switched to, so the hook can
   *  emit a "switching back" cue on the following run. */
  ...
  const run = useCallback(
    async (userContent: UserContent) => {
      ...
      const runSingle = async (
        ...
        // On Windows/WSL, Ink's live-area redraws snap the terminal viewport
        // back to the bottom, so the flush is throttled harder there — still
        // responsive, but far less scroll-jumping.
        const isWinOrWSL = process.platform === "win32" || /microsoft|WSL/i.test(os.release());
        const STREAM_FLUSH_MS = isWinOrWSL ? 200 : 100;
