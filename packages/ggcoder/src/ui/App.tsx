...
import { isEyesActive } from "@abukhaled/ggcoder-eyes";
...
export function App(props: AppProps) {
  ...
  useEffect(() => {
    if (props.settingsFile) {
      ...
      sm.load().then(() => {
        ...
        // `buddyEnabled` is a fork-only setting that is not in the shared
        // Settings type, so read it the same way /buddy writes it
        // (core/slash-commands.ts) — through the untyped record view.
        const raw = sm.getAll() as unknown as Record<string, unknown>;
        setBuddyEnabled(raw.buddyEnabled === true);
      });
    }
  }, [props.settingsFile]);
  ...
  // Per-turn model router. Auto-switches to a vision-capable model when the
  // turn carries images and switches back afterwards. Logged-in providers and
  // their credentials are passed so the router can borrow another provider's
  // vision model for a single turn (e.g. MiniMax -> Claude / GLM-4.6V).
  ...
  const handleSubmit = useCallback(
    async (input: string, inputImages: ImageAttachment[] = [], pasteInfo?: PasteInfo) => {
      ...
      // Full-screen task manager (create/edit/run). Ctrl+T opens the inline
      // picker; this pane is the richer editor view.
      if (trimmed === "/tasks") {
        setOverlay("tasks");
        return;
      }

      // Open the Eyes pane — read-only review of installed probes + open
      // signals. Gated by the ggcoder-eyes manifest: in projects without
      // /setup-eyes there is nothing useful to show.
  ...
  const allCommands = useMemo<SlashCommandInfo[]>(() => {
    ...
    return [
      ...
      { name: "tasks", aliases: [], description: "Open task manager", sectionTitle: "built-in" },
      {
        name: "eyes-view",
        aliases: ["ev"],
        description: "Review Eyes probes and open signals",
  ...
  return (
    <Box flexDirection="column" width={columns} flexShrink={0} flexGrow={0}>
      ...
      {/* Buddy companion — opt-in via settings, sits just under the composer. */}
      {buddyEnabled && !overlay && <Buddy phase={agentLoop.activityPhase} />}
    </Box>
  );
}
