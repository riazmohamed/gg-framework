...
export function createAutoUpdater(config: AutoUpdateConfig): AutoUpdater {
  ...
  const periodicMessage =
    ...
    (({ currentVersion, latestVersion, updateCommand }) =>
      `A fresh update just landed — ${currentVersion} → ${latestVersion}! I'll grab it on next launch (or run ${updateCommand} if you can't wait).`);
      ...
      if (state?.updatePending && state.latestVersion) {
        if (compareVersions(state.latestVersion, currentVersion) > 0) {
          ...
          if (info.updateCommand) {
            ...
            message = `Version ${state.latestVersion}! Installing in the background — takes effect next launch.`;
            writeState({
              ...state,
              lastCheckedAt: Date.now(),
              updatePending: false,
