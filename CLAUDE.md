...
outside React so it is unit-testable), `QueuedBar` (prompts queued behind a running turn),
...
- **Verification gate** (`core/verification-gate.ts`). If a run mutated code files and no
  test / typecheck / lint / build ran *after the last mutation*, a pre-stop hook injects
  **exactly one** follow-up demanding the project's verification. One, deliberately: a gate
  that keeps prompting past the model's decision to stop only buys extra full-length final
  answers, so the demand carries its own fallback ("say which changes went unverified") and
  the gate then goes silent. Recognition is a conservative runner-shape classifier applied
  per segment of a compound command — **both error directions are safe**: a miss leaves the
  gate silent (the old behavior), a false positive skips one continuation.
