...
**Workspace globs** (`pnpm-workspace.yaml`): `packages/*`, `experiments/*`, `gg-app`. `Matey/` is intentionally isolated from the root workspace with its own workspace file and lockfile; drive it via `pnpm --dir Matey <script>` from the repo root.
